...
export class SignInPage implements OnInit {
 ...
 // public address: any;
  ...
  URL='http://vvdconnect.democodes.in/secures/';
  ...
  form_messages = {
    'mobileNumber': [
      ...
      { type: 'maxlength', message: 'Mobile number requires atleast 10 characters'},
      { type: 'minlength', message: 'Mobile number requires atleast 10 characters'}
    ],
    'password': [
 ...
 goToOtp() {
  ...
  registernow()
  {
    ...
    else
    {
     ...
     let result= this.http.post(this.Gooole_Api+'Getsalesmaster', JSON.stringify(this.registerForm.value),{headers:headers}).subscribe(data => {
  ...
  this.http.post(this.API_URL+'register', formcontactData,{headers: headers})
  .subscribe(data => {
    console.log(data);
    this.storage.set('registerdata',data);
    this.loading.dismiss();
    ...
    console.log('clickkk','cliccc1111');
     ...
     .then((result: NativeGeocoderResult[]) => {
       ...
       this.geoAddress = this.generateAddress(result[0]);
        ...
        this.address=result[0]['areasOfInterest']+","+result[0]['subLocality'];
 ...
 if(getDocument=="")
 {
   ...
   console.log("iffgetvaass",getDocument);
    ...
    if(this.registerForm.invalid)
    {
      console.log('Error');
      this.loading.dismiss();
      this.fun.presentToast('Fill the all mandate fields!', true, 'bottom', 2100);
    }
    else
   {
      if(this.ImageUploadCheck ==2)
      {
       this.fun.presentToast('Please Upload Retaile Image', true, 'bottom', 2100);
...
this.loading.present();
     ...
     let result= this.http.post(this.API_URL + "newregister", body)
      .subscribe(data => {
  ...
  goTohome() {
    ...
    var headers = new HttpHeaders();
      headers.append("Accept", 'application/json');
      headers.append('Content-Type', 'application/json' );
      console.log(this.API_URL+'loginnew', JSON.stringify(this.loginForm.value));
      let result = this.http.post(this.API_URL+'loginnew', JSON.stringify(this.loginForm.value)).subscribe(data =>{
        if(data)
        {
          ...
          if(data['response']['code']==1)
          {
            ...
            localStorage.setItem('loggeduserid',data['response']['data']['userId']);
            localStorage.setItem('firstName',data['response']['data']['ret_name']);
        ...
        let result = this.http.post(this.API_URL + 'loginnew', JSON.stringify(this.loginForm.value)).subscribe(data => {
          if (data) {
            ...
            if (data['response']['code'] == 1) {
              ...
              localStorage.setItem('apikey', data['response']['data']['apikey']);
              localStorage.setItem('loggeduserid', data['response']['data']['userId']);
              localStorage.setItem('ret_name', data['response']['data']['ret_name']);
              localStorage.setItem('mobile', data['response']['data']['mobile']);
              localStorage.setItem('available_points', data['response']['data']['available_points']);
              console.log(localStorage.getItem('apikey'));
              console.log(localStorage.getItem('mobile'));
              //this.storage.set('registerdata',datas);   
              console.log('currentUser', data['response']['data']);
              this.storage.set('currentUser', JSON.stringify(data)).then((datas) => {
            ...
            else
            {
              ...
              this.fun.presentToast(data['response'][0]['message'], true, 'bottom', 2100);
