...
export class ShippingPage implements OnInit {
  ...
  constructor(private router: ActivatedRoute, private fun: FunctionsService, private http: HttpClient, public loading: LoadingService, private geolocation: Geolocation, private route: Router, private authService: AuthenticationService) { }
  ...
  async callMyaddressapi() {
    ...
    await this.authService.getMyaddress(this.getApikey).subscribe(async res => {
  ...
  add_address() {
    ...
    this.route.navigate(['./addaddress', { directopt: 0,finalprice:this.finalprice,couponCode:this.couponCode,advancedAmount:this.advancedAmount }]);
  }
  payment() {

    this.route.navigate(['./payment', { addressid: this.getid ,finalprice:this.finalprice,couponCode:this.couponCode,advancedAmount:this.advancedAmount}]);
  }
}
