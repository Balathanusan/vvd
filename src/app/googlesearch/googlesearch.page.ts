...
export class GooglesearchPage implements OnInit {
  ...
  place: any;
  APIcall = environment.API + "mapgeocode?";
  getlat: any;
  getlng: any;
  getshrtname: any;
  ...
  callMapapi(placeid) {
    ...
    return this.http
      .get(this.APIcall + "placeId=" + placeid + "&APIKEY=" + this.getApikey, {
        headers: headers,
      })
    ...
    this.callMapapi(placeid).subscribe(async (res) => {
      ...
      if (this.getstatus == "success") {
        ...
        this.route.navigate([
          "./addaddress",
          {
            getformattedaddrs: this.getformattedaddrs,
            getlat: this.getlat,
            getlng: this.getlng,
            getshrtname: this.getshrtname,
            directopt: 1,
          },
        ]);
      } else {
