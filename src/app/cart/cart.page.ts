...
export class CartPage implements OnInit {
  ...
  constructor(
    private route: Router, public db: DatabaseProvider,
    private modalController: ModalController, private http: HttpClient, private fun: FunctionsService, public loading: LoadingService,
  ...
  increment(index, id) {
    ...
    this.db.getRecordByProductID(id).subscribe((res: any) => {
      ...
      if (res.length == 0) {
        let data = { id: id, loggeduserid: this.userId, itemQuantity: this.val };
        console.log(data);
  ...
  decrement(index, id) {
    ...
    if (this.decreaseval < 0) {
      ...
      let printupdate = this.db.updateRecordRow(id, this.userId, this.decreaseval);
      let currentPrice = this.cartProducts[index].singleprice;
  ...
  async getSepidfromDB() {
    ...
    if (this.getcartvalue.length > 0) {
      ...
      for (let index in apiDatas) {
        ...
        let countQty = await this.getProductCountFromArray(productId, this.dataDB);
        if (countQty > 0) {
 ...
 async coupon_code() {
  ...
  shipping() {
    ...
    this.getcartdetail = [];
    for (var i = 0; i < this.cartProducts.length; i++) {
      this.getcartdetail.push({ productId: this.cartProducts[i].id, qty: this.cartProducts[i].currentItems });
    }
    console.log
    this.getfinalprice = 0;
    this.getcountfromdb = 0;
    ...
    this.route.navigate(['./shipping', {  finalprice: this.grantTotalValue, couponCode: couponcode,advancedAmount:this.advancedAmount}]);
  }
  goToHome() {
