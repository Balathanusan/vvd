...
export class DatabaseProvider {
  ...
  tableInit() {
    this.databaseObj
      .executeSql(
        `create table ${this.table} (id int,loggedUserid text null,itemQuantity int null,filterParams BLOB)`,
        []
      )
  ...
  public insertRecordRow(data) {
    ...
    this.databaseObj
      .executeSql(insert, [data.id, data.loggeduserid, data.itemQuantity])
      .then((res) => {
  ...
  public deleteAllRecordRid(loggedUserid) {
    ...
    this.databaseObj
      .executeSql(deleteRow, [loggedUserid])
      .then((res) => {
  ...
  public updateRecordRow(id, loggedUserid, itemq) {
    ...
    return this.databaseObj
      .executeSql(
        `UPDATE  ${this.table} SET itemQuantity = ? WHERE id = ${id} and loggedUserid = ${loggedUserid}`,
        data
      )
      .then((data) => {});
  }
  ...
  public getItemCount(loggeduserid) {
    ...
    return new Observable((observer) => {
      this.databaseObj
        .executeSql(select, [loggeduserid])
        .then((res) => {
          observer.next(res);
        })
        .catch((e) => {
  ...
  public getRowCount(id, loggeduserid) {
    ...
    return new Observable((observer) => {
      this.databaseObj
        .executeSql(select, [id, loggeduserid])
        .then((res) => {
          observer.next(res);
        })
        .catch((e) => {
  ...
  public insertSelectedFilterIDs(id, selectedIDs) {
    ...
    this.databaseObj
      .executeSql(insert, [id, selectedIDs])
      .then((res) => {
        console.log("insertres" + JSON.stringify(res));
        console.log("Row Inserted!");
      })
